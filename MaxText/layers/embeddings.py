...
class RotaryEmbedding(nn.Module):
  ...
  def __call__(
    ...
    self.sow('intermediates', 'timescale', timescale)
    self.sow('intermediates', 'position', position)

    position = position[:, :, jnp.newaxis, jnp.newaxis]
    sinusoid_inp = position / timescale

    self.sow('intermediates', 'sinusoid_inp', sinusoid_inp)

    sin = jnp.sin(sinusoid_inp).astype(inputs.dtype)
    cos = jnp.cos(sinusoid_inp).astype(inputs.dtype)

    self.sow('intermediates', 'sin', sin)
    self.sow('intermediates', 'cos', cos)
