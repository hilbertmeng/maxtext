...
def run(config):
  ...
  use=qktype_scenarios() if stage=='qktype' else vdepth_scenarios() if stage=='vdepth' else depth_scenarios() if stage=='depth' else targeted_scenarios() if stage=='targeted' else [s for s in all_scenarios if stage=='all' or (stage=='groups' and s['kind'] in ('coalition','dose')) or (stage=='layers' and s['kind'] in ('layer','unit'))]
  assert use and stage in ('all','groups','layers','targeted','qktype','vdepth','depth')
