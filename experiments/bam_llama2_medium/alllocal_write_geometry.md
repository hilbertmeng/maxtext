...
## 6a. 不筛选写门时：读门、O占比、写门的三者关系

这部分专门使用全部128条序列中 localO 非零的位置；**没有用 read-O gate 或 write gate 做筛选**。L1–L23共保留约2,100万条位置，关系图和逐头统计在 `worker/unfiltered_gate_share/`。为控制内存， pooled 图使用按层×头截断的均衡 reservoir；逐层逐头 Spearman 是完整位置统计。

最强关系是 `read_o_gate → localO norm share`：各头 Spearman 的中位数为 **0.792**，10%–90%范围 **0.615–0.943**，所有头都是正相关。这个结果有一部分不是模型学出来的规律，而是计算结构：`local_o = read_o_gate × raw_local_o`，而 O share 的分母包含四项模长。因此在其他量固定时，读门变大必然抬高 O 的相对占比。它说明读门确实决定 O 在当前写入向量中的份量，但不能直接解释为“读门发现了更重要的 O 内容”。

`localO share → write_gate` 才是更有区分度的问题。未经控制的头内相关中位数为 **−0.235**；L3–L18大部分头为负，L21–L22又转为偏正。控制 read gate、层和头的秩残差相关仍为 **−0.373**；反过来控制 O share 后，`read gate → write gate` 为 **+0.201**。这给出一个很具体的分解：在同样的读门开度下，O占比更高通常对应更低的写门；在同样的O占比下，读门本身与写门又有弱正关联。两者不能合并成“读门越大所以写门越大”。这些是观测相关，不是因果效应。

分层后，关系明显是层/头特异的：L5–L18多数头的 O share→write 仍为负，L1–L2较混合，L21–L22接近零或转正。完整二维图显示读门与 O share 的强弯曲带，而 write gate 在相同区域还受层头基线和其它分量影响。

![不筛选三者关系](figures/alllocal_write_geometry/unfiltered_gate_share.png)

这也是为什么本次不能沿用“双门同时打开”的筛选：那会把 write gate 的低值区直接切掉，也会把 read gate→O share 的机械关系截断在高门区域，无法回答三者如何共同决定“是否写”。当前分析仍然是同一前向中的关联；要证明写门真的根据 O 占比做选择，需要固定输入表示或做配对干预。
