...
def check_optimizer_serialization_contract(with_rules):
  cfg = SimpleNamespace(
      wd_mults=exp.BamLlama2MediumV2C256RmsGeluAlphaMix.wd_mults if with_rules else None,
  ...
  for step in (0, 1, 199, 200, 201, 2800, 13499, 13500):
    ...
    if step == 199:
      ...
      if with_rules:
        np.testing.assert_array_equal(actual['fetch_mix_scale'], leaves['fetch_mix_scale'])
        np.testing.assert_array_equal(actual['W_lq_bias'], leaves['W_lq_bias'])
      else:
