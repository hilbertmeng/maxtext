...
def intervene(target_layer,target_head,threshold,shift,zero_cutoff=None,target_probability=-1.,probability_delta=None,gate_lower=0.,gate_upper=1.000001):
 stack=[]
 def hook(next_fun,args,kw,ctx):
  ...
  selected=selected&(gf>=gate_lower)&(gf<gate_upper)
...
def run(cfg):
 ...
 cached=None
 if os.environ.get('BET_GRADIENT_CACHE'):
  assert zero_cutoff is None, 'Cache is for adaptive probability-space directions'
  cachepath=Path(os.environ['BET_GRADIENT_CACHE'])
  with np.load(cachepath) as c:
   assert str(c['protocol_sha256'])==hashlib.sha256(Path(os.environ['BET_PROTOCOL']).read_bytes()).hexdigest()
   for key in ['cohort_sha256','model','checkpoint','dtype']:assert str(c[key])==meta[key],key
   assert list(c['head_ids'])==[h['id'] for h in protocol['heads']]
   cached=c['values'];cached_primal=c['primal_delta']
   meta.update(gradient_source='cached per-position dL/dg projected onto fixed adaptive steps',gradient_source_runtime=str(c['gradient_source_runtime']),gradient_cache_sha256=hashlib.sha256(cachepath.read_bytes()).hexdigest())
 ...
 def forward(p,b,l,h,t,f,enabled=True,target_probability=-1.,gate_lower=0.,gate_upper=1.000001):
  with intervene(l,h,t,f,zero_cutoff=zero_cutoff,target_probability=target_probability,gate_lower=gate_lower,gate_upper=gate_upper) if enabled else contextlib.nullcontext():
 ...
 with mesh,partitioning.axis_rules(cfg.logical_axis_rules):
  ...
  for i in range(start_sequence,stop_sequence):
   ...
   for head_index,head in enumerate(protocol['heads']):
    if cached is not None:
     derivative,selected_count,step_sum=cached[i,head_index]
     gradients.append(dict(id=head['id'],derivative=float(derivative),primal_delta=float(cached_primal[i]),selected=float(selected_count),step_sum=float(step_sum),source='cached_per_position'))
     continue
    (gloss,(gstats,gtokens)),derivative=jax.device_get(gradfn(state.params,b,jnp.asarray(head['layer']),jnp.asarray(head['head']),jnp.asarray(head['threshold']),jnp.asarray(0.),gate_lower=jnp.asarray(head.get('gate_lower',0.)),gate_upper=jnp.asarray(head.get('gate_upper',1.000001))))
   ...
   for j in order:
    a=arms[j];loss,(stats,tokens)=jax.device_get(fn(state.params,b,jnp.asarray(a['layer']),jnp.asarray(a['head']),jnp.asarray(a['threshold']),jnp.asarray(a['shift']),target_probability=jnp.asarray(a.get('target_probability',-1.)),gate_lower=jnp.asarray(a.get('gate_lower',0.)),gate_upper=jnp.asarray(a.get('gate_upper',1.000001))))
    token_rows[j]=tokens
