...
if (( $# < 8 )); then
  echo "usage: $0 CONFIG TRAINER_COMMIT RUN_PREFIX CHECKPOINT_BASE OUTPUT_BASE RANKS STEP..." >&2
  exit 2
fi
...
CODE_COMMIT="${BAM_ABSV_SUBSPACE_DIAGNOSTIC_COMMIT:-$(git rev-parse HEAD)}"
