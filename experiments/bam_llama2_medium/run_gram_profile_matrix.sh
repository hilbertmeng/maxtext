...
case "$region" in
  ...
  europe-west4|us-east5) dataset=gs://newproject-1-common_datasets_${region}/pythia_pile_idxmaps_tfrecord ;;
  *) echo "ERROR: no Pile replica registered for $region" >&2; exit 2 ;;
esac
commit=$3
label=$4
shift 4
...
for exp in "$@"; do
  ...
  "${gcloud_base[@]}" compute tpus tpu-vm ssh --internal-ip "$tpu" --zone="$zone" \
    ...
    sudo rm -f /tmp/libtpu_lockfile; cd '$repo'; nohup env DATASET_PATH='$dataset' SMOKE_OUTPUT='$remote_run' \
