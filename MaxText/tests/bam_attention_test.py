...
class BamReadKeyTransformTest(absltest.TestCase):
  ...
  def test_independent_erase_gates_formula_and_health(self):
    import copy
    import max_utils
    import pyconfig
    from flax.core import unfreeze
    ...
    with tempfile.TemporaryDirectory() as out:
      Path(out, 'edges').mkdir()
      cfg=pyconfig.initialize([None,str(Path(__file__).parents[1]/'configs/base.yml')],
          exp_class='BamMediumAllLocalIndependentErase',run_name='edges',
          enable_checkpointing=False,base_output_directory=out+'/',jax_cache_dir='',
          log_config=False,dataset_type='synthetic',base_emb_dim=128,
          base_num_query_heads=2,base_num_kv_heads=2,head_dim=64,max_target_length=8,
          max_prefill_predict_length=8,query_chunk_size=4,per_device_batch_size=1.,dtype='float32')
      ...
      for dtype in (jnp.float32,jnp.bfloat16):
        ...
        bias=params['feedback_gw_b0']
        params['feedback_gw_b0']=bias.replace(value=jnp.full_like(bias.value,jnp.log(.3/.7)))
        self.assertIs(bound.W_erase.kernel_init,bound.W_gw.kernel_init)
        self.assertFalse(np.array_equal(nn.unbox(params['W_erase']['kernel']),nn.unbox(params['W_gw']['kernel'])))
        np.testing.assert_array_equal(nn.unbox(params['erase_gate_b0']),nn.unbox(params['gw_b0']))
        outputs=[]
        for opening in (.001,.2,.9):
          p=copy.deepcopy(params);bias=p['W_R_gate_b0']
          p['W_R_gate_b0']=bias.replace(value=jnp.full_like(bias.value,jnp.log(opening/(1-opening))))
        ...
        # Each write gate affects memory, but neither changes this layer's residual output.
        base=module.apply({'params':params},*args,**kw)
        for name in ('gw_b0','feedback_gw_b0','erase_gate_b0'):
          p=copy.deepcopy(params);bias=p[name];p[name]=bias.replace(value=bias.value+.5)
          changed=module.apply({'params':p},*args,**kw)
          np.testing.assert_array_equal(changed[0],base[0])
          self.assertGreater(float(jnp.linalg.norm((changed[1]-base[1]).astype(jnp.float32))),0)
        if dtype == jnp.float32:
          ...
          np.testing.assert_array_equal(nn.unbox(grad['W_R_gate']['kernel']),0.)
          np.testing.assert_array_equal(nn.unbox(grad['W_R_gate_b0']),0.)
          for name in ('W_gw','W_gw_feedback','W_erase'):
            value=nn.unbox(grad[name]['kernel'])
            self.assertTrue(bool(jnp.all(jnp.isfinite(value))))
            self.assertGreater(float(jnp.linalg.norm(value)),0)
          # Independent RMS formula and health must refer to the actual two writes.
          ...
          r=jax.random.normal(jax.random.key(126),(1,8,2,64))
          local=.4*r
          read_address=jax.random.normal(jax.random.key(128),(1,8,2,32))
          actual,health=module.apply({'params':params},u+local,x,memory,
              local_o=local,local_o_raw=r,non_local_o=u,local_read_address=read_address,method=module._write,mutable=['intermediates'])
          bound=module.bind({'params':params})
          un=bound.write_data_norm(u[...,:48]);rn=bound.write_data_norm(r[...,:48])
          g=jax.nn.sigmoid(bound.W_gw(x)+bound.gw_b0)
          f=jax.nn.sigmoid(bound.W_gw_feedback(x)+bound.feedback_gw_b0)
          address=bound.write_address_norm(bound.P_loc_up(nn.gelu(bound.P_loc_down(x))))
          e=jax.nn.sigmoid(bound.W_erase(x)+bound.erase_gate_b0)
          read_normal=bound.write_address_norm(read_address)
          expected=memory+jnp.einsum('btnk,btnv->btkv',g[...,None]*un+f[...,None]*rn,address)-jnp.einsum('btnk,btnv->btkv',e[...,None]*rn,read_normal)
          np.testing.assert_allclose(actual[0],expected,rtol=1e-5,atol=1e-6)
          n=jnp.linalg.norm(g[...,None]*un,axis=-1)*jnp.linalg.norm(address,axis=-1)
          b=jnp.linalg.norm(f[...,None]*rn,axis=-1)*jnp.linalg.norm(address,axis=-1)
          expected_share=jnp.mean(b,axis=(0,1))/jnp.mean(n+b,axis=(0,1))
          np.testing.assert_allclose(health['intermediates']['dual_write_health'][0][:,6],expected_share,rtol=1e-5)
          stats=health['intermediates']['erase_health'][0]
          self.assertEqual(stats.shape,(2,13))
          erase_norm=jnp.linalg.norm(e[...,None]*rn,axis=-1)*jnp.linalg.norm(read_normal,axis=-1)
          net_norm=jnp.linalg.norm(rn,axis=-1)*jnp.linalg.norm(f[...,None]*address-e[...,None]*read_normal,axis=-1)
          np.testing.assert_allclose(stats[:,4],jnp.mean(b,axis=(0,1))/jnp.mean(n+b+erase_norm,axis=(0,1)),rtol=1e-5)
          np.testing.assert_allclose(stats[:,5],jnp.mean(erase_norm,axis=(0,1))/jnp.mean(n+b+erase_norm,axis=(0,1)),rtol=1e-5)
          np.testing.assert_allclose(stats[:,7],jnp.mean(net_norm,axis=(0,1))/jnp.mean(n+net_norm,axis=(0,1)),rtol=1e-5)
          # Close each write independently: erasure need not shrink when positive feedback grows.
          for name in ('feedback_gw_b0','erase_gate_b0'):
            q=copy.deepcopy(params);leaf=q[name];q[name]=leaf.replace(value=jnp.full_like(leaf.value,-100.))
            changed,inter=module.apply({'params':q},u+local,x,memory,local_o=local,local_o_raw=r,
                non_local_o=u,local_read_address=read_address,method=module._write,mutable=['intermediates'])
            if name=='feedback_gw_b0':
              np.testing.assert_allclose(inter['intermediates']['erase_health'][0][:,0],stats[:,0])
              expect=memory+jnp.einsum('btnk,btnv->btkv',g[...,None]*un,address)-jnp.einsum('btnk,btnv->btkv',e[...,None]*rn,read_normal)
            else:
              np.testing.assert_allclose(inter['intermediates']['dual_write_health'][0][:,1],jnp.mean(f,axis=(0,1)))
              expect=memory+jnp.einsum('btnk,btnv->btkv',g[...,None]*un+f[...,None]*rn,address)
            np.testing.assert_allclose(changed[0],expect,rtol=1e-5,atol=1e-6)
      for key,value in dict(num_decoder_layers=3,vocab_size=128,
          bam_layer_modes=['local_qk+local_v+local_o']*3,mlp_dim_by_block=[128]*3).items():cfg.get_keys()[key]=value
      model=models.Transformer(config=cfg,mesh=mesh,quant=None)
      tokens=jnp.ones((1,8),jnp.int32);positions=jnp.arange(8)[None]
      ...
      self.assertIn('bam/dual_write/layer_002/head_01/feedback_mean',emitted['scalar'])
      self.assertIn('bam/raw_write/layer_002/feedback_record_norm_rms',emitted['scalar'])
      self.assertIn('bam/erase/layer_002/head_01/erase_mean',emitted['scalar'])

  def test_feedback_address_mix_lifting_independence_and_actual_write_health(self):
    import copy
    import max_utils
    import pyconfig
    from flax.core import unfreeze
    ...
    with tempfile.TemporaryDirectory() as out:
      Path(out, 'edges').mkdir()
      cfg=pyconfig.initialize([None,str(Path(__file__).parents[1]/'configs/base.yml')],
          exp_class='BamMediumAllLocalIndependentEdgesAddressMix',run_name='edges',
          enable_checkpointing=False,base_output_directory=out+'/',jax_cache_dir='',
          log_config=False,dataset_type='synthetic',base_emb_dim=128,
          base_num_query_heads=2,base_num_kv_heads=2,head_dim=64,max_target_length=8,
          max_prefill_predict_length=8,query_chunk_size=4,per_device_batch_size=1.,dtype='float32')
      ...
      for dtype in (jnp.float32,jnp.bfloat16):
        ...
        bias=params['feedback_gw_b0']
        params['feedback_gw_b0']=bias.replace(value=jnp.full_like(bias.value,jnp.log(.3/.7)))
        np.testing.assert_array_equal(nn.unbox(params['W_feedback_address_mix']['kernel']),0.)
        np.testing.assert_array_equal(nn.unbox(params['feedback_address_mix_gate_b0']),0.)
        bound=module.bind({'params':params})
        _,_,raw,lifted=bound._independent_local_vo(memory,x)
        # Non-square C8->full32 lifting must reproduce the actual compressed read.
        direct=jnp.einsum('btkv,btnv->btnk',memory,lifted)*bound._read_key_scale
        np.testing.assert_allclose(raw[...,:48].astype(jnp.float32),direct.astype(jnp.float32),
                                   rtol=.04 if dtype==jnp.bfloat16 else 1e-5,
                                   atol=.05 if dtype==jnp.bfloat16 else 1e-5)
        outputs=[]
        for opening in (.001,.2,.9):
          p=copy.deepcopy(params);bias=p['W_R_gate_b0']
          p['W_R_gate_b0']=bias.replace(value=jnp.full_like(bias.value,jnp.log(opening/(1-opening))))
        ...
        # Each write gate affects memory, but neither changes this layer's residual output.
        base=module.apply({'params':params},*args,**kw)
        for name in ('gw_b0','feedback_gw_b0','feedback_address_mix_gate_b0'):
          p=copy.deepcopy(params);bias=p[name];p[name]=bias.replace(value=bias.value+.5)
          changed=module.apply({'params':p},*args,**kw)
          np.testing.assert_array_equal(changed[0],base[0])
          self.assertGreater(float(jnp.linalg.norm((changed[1]-base[1]).astype(jnp.float32))),0)
        if dtype == jnp.float32:
          ...
          np.testing.assert_array_equal(nn.unbox(grad['W_R_gate']['kernel']),0.)
          np.testing.assert_array_equal(nn.unbox(grad['W_R_gate_b0']),0.)
          for name in ('W_gw','W_gw_feedback','W_feedback_address_mix'):
            value=nn.unbox(grad[name]['kernel'])
            self.assertTrue(bool(jnp.all(jnp.isfinite(value))))
            self.assertGreater(float(jnp.linalg.norm(value)),0)
          # Independent RMS formula and health must refer to the actual two writes.
          ...
          address=bound.write_address_norm(bound.P_loc_up(nn.gelu(bound.P_loc_down(x))))
          read_normal=bound.write_address_norm(read_address)
          mixed=.5*(address-read_normal)
          expected=memory+jnp.einsum('btnk,btnv->btkv',g[...,None]*un,address)+jnp.einsum('btnk,btnv->btkv',f[...,None]*rn,mixed)
          np.testing.assert_allclose(actual[0],expected,rtol=1e-5,atol=1e-6)
          n=jnp.linalg.norm(g[...,None]*un,axis=-1)*jnp.linalg.norm(address,axis=-1)
          b=jnp.linalg.norm(f[...,None]*rn,axis=-1)*jnp.linalg.norm(mixed,axis=-1)
          expected_share=jnp.mean(b,axis=(0,1))/jnp.mean(n+b,axis=(0,1))
          np.testing.assert_allclose(health['intermediates']['dual_write_health'][0][:,6],expected_share,rtol=1e-5)
          stats=health['intermediates']['feedback_address_health'][0]
          self.assertEqual(stats.shape,(2,8))
          np.testing.assert_allclose(stats[:,0],.5)
          self.assertTrue(bool(jnp.all(jnp.isfinite(stats))))
          # Feedback-only negative-read endpoint weakens the content along the read direction.
          read_content=jnp.einsum('btkv,btnv->btnk',memory,read_address)
          padded=jnp.pad(read_content,((0,0),(0,0),(0,0),(0,16)))
          q=copy.deepcopy(params)
          for name,value in [('gw_b0',-100.),('feedback_address_mix_gate_b0',-100.),('feedback_gw_b0',-8.)]:
            leaf=q[name];q[name]=leaf.replace(value=jnp.full_like(leaf.value,value))
          erased=module.apply({'params':q},padded,x,memory,local_o=jnp.zeros_like(padded),
              local_o_raw=padded,non_local_o=jnp.zeros_like(padded),local_read_address=read_address,
              method=module._write)[0]
          # Aggregate energy inner product is negative even with cross-head interactions.
          self.assertLess(float(jnp.sum(memory*(erased-memory))),0.)
          # Exact zero reads remain finite, including the newly added address branch.
          zero_result=module.apply({'params':params},jnp.zeros_like(u),x,jnp.zeros_like(memory),
              local_o=jnp.zeros_like(u),local_o_raw=jnp.zeros_like(u),non_local_o=jnp.zeros_like(u),
              local_read_address=jnp.zeros_like(read_address),method=module._write)[0]
          np.testing.assert_array_equal(zero_result,0.)
      for key,value in dict(num_decoder_layers=3,vocab_size=128,
          bam_layer_modes=['local_qk+local_v+local_o']*3,mlp_dim_by_block=[128]*3).items():cfg.get_keys()[key]=value
      model=models.Transformer(config=cfg,mesh=mesh,quant=None)
      tokens=jnp.ones((1,8),jnp.int32);positions=jnp.arange(8)[None]
      ...
      self.assertIn('bam/dual_write/layer_002/head_01/feedback_mean',emitted['scalar'])
      self.assertIn('bam/raw_write/layer_002/feedback_record_norm_rms',emitted['scalar'])
      self.assertIn('bam/feedback_address/layer_002/head_01/mix_mean',emitted['scalar'])
