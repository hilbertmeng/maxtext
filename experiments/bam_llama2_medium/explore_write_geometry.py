...
def run(task):
 ...
 for ti,t in enumerate(thresholds):
  ...
  for h in range(16):
   ...
   conditions=[m,good,good&(cc<0),good&(a < -1),good&(a>0),good&(a<=0)&(a>=-1),good&(rr>1)]
 ...
 for i in range(n):
  for h in range(16):
   ...
   xx.append(np.column_stack([np.full(len(ids),h),np.log1p(pos[i,ids]),go[i,ids,h],gv[i,ids,h],z]));yy.append(gw[i,ids,h]);split.extend([i<32]*len(ids));sp.extend([i]*len(ids))
 ...
 if xx:
  ...
  groups={'head_position':[0,1],'read_gate_controls':[0,1,2,3],'amplitude':[0,1]+list(range(4,8)),'composition':[0,1]+list(range(8,12)),'angles':[0,1]+list(range(12,18)),'geometry':[0,1]+list(range(4,20)),'geometry_and_read_gates':list(range(20))}
  preds={}
