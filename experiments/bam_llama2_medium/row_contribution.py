...
class RowContributionProbe(getattr(exp,BASE)):
  ...
  load_parameters_path=f'gs://newproject-1-llm_projects_europe-west4/log/{BASE}/checkpoints/30091/items'
...
def row_interventions(scales):
  ...
  def intercept(next_fun,args,kw,ctx):
    ...
    if isinstance(ctx.module,attentions.BamAttention) and ctx.method_name=='_read_fetched_m':
      ...
      # Independent XL has 16 fetched heads == 16 MHA heads. Packed tail
      # begins at bam_k=64: eight compressed row coordinates then padding.
...
def run(config):
  ...
  metadata=dict(model=BASE,checkpoint=config.load_parameters_path,
      training_commit='c664e8211c299c9c780ce1667553cf995130c453',
      ...
      model_shape={k:getattr(config,k) for k in ('emb_dim','num_decoder_layers','num_query_heads','head_dim','bam_k','bam_v','bam_local_q_rank','bam_local_v_rank','bam_local_qk_share_basis')},
