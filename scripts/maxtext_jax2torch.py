...
def update_weight_from_maxtext(model, w, vocab_size=50304, num_blocks=2, model_dim=1024, num_heads=16, dtype=torch.bfloat16, moe=False):
    ...
    for k, v in model.named_parameters():
        if k == 'tok_embeddings.weight':
            ...
            if '.attention.' in k:
                ...
                ptype = k.split('.')[-1] # w1, w3,w2,mgate_layer
                if moe:
                    if 'router_gate' in k:
                        v = w[f'decoder.layers.unshared_mlp_{sub_layer}.router_gate.kernel'][:,_layer] # GeneralDense have .kernel, and torch.nn.parameters don't need transpose, torch.nn.Linear need.
                    else:
                        v = w[f'decoder.layers.unshared_mlp_{sub_layer}.{map_dict[ptype]}'][:,_layer] # self.params no .kernel, and torch.nn.parameters don't need transpose, torch.nn.Linear need.
                else:
...
# 8B Dense模型大约需要80G内存, 31B的moe需要240内存
# read_dir = f'gs://llm_base_models_us-central2/dcformer/maxtext/410m/qknorm0511_scale/checkpoints/6000/default'
# orbax-checkpoint==0.5.20, jax>=0.4.30
if __name__ == '__main__':
    '''
    run command:
    python maxtext_jax2torch.py --model_path  gs://bucket/dcformer/checkpoints/6000/default --output_dir ./dense_latest_torch
    or
    python maxtext_jax2torch.py --model_path gs://llm_base_models_europe-west4/v5p_256/7B/xm_E8S0T2A11B_CopyInit_0102/checkpoints/151050/state --output_dir v4moe_latest_torch --moe
    '''
    ...
    parser.add_argument("--moe", action='store_true', default=False) # model directory, such as cloud path(gs://bucket/dcformer/checkpoints) or local path(./dcformer/checkpoints)
    parser.add_argument("--output_dir", required=True) # model directory, such as cloud path(gs://bucket/dcformer/checkpoints) or local path(./dcformer/checkpoints)
    ...
    # DCFormer-Medium
    config = {"torch_dtype": torch_dtype, "prefill_pad": True, "vocab_size": 152064,"n_layer": 48, "n_head":32, "dim": 4096, "use_qk_norm": True, "window_type": "LGLL", "window_size": 256, "rope_base":10000, "intermediate_size": 5632, 'mgate': True}
    moe_extra_config = {"min_group_size": 1, "num_experts": 8, "mgate_dim": 44, "num_experts_per_tok": 2, "expert_capacity_factor": 2, "gate_noise_coef": 0, "sfm_after_topn": True, "aux_loss_coef": None, "router_z_loss_coef": None, "expert_chunk_size": 8, 'int8': False}
    if args.moe:
        print(f'MOE model convert......')
        config.update(moe_extra_config)

    start_time = time.time()
    print('Start to init torch model.')
    config = DCFormerConfig(**config)
    model = DCFormer(config)
    print(f'Init torch model finished. take: {time.time() - start_time:.3f}s \n')
    print(f'Torch model params:')
    for k, v in model.named_parameters():
        print(k, v.shape)
    print('\n\n')

    start_time = time.time()
    print('Start to load jax weights')
    read_dir = args.model_path
    print('read_dir', read_dir)
    weights = load_model(read_dir)
    print(f'Load jax weights finished. take: {time.time() - start_time:.3f}s\n')
    print(f'Jax model params:')
    for k, v in weights.items():
        print(k, v.shape)
    print('\n\n')

    start_time = time.time()
    print('Start to convert weights')
    # convert maxtext model weight to pytorch model weight
    model = update_weight_from_maxtext(model, weights, vocab_size=152064, num_blocks=4, model_dim=4096, num_heads=32, dtype=config.torch_dtype, moe=args.moe)
    print(f'Convert weights finished. take: {time.time() - start_time:.3f}s\n')

    start_time = time.time()
    print('Start to save torch weights.')
    if 'bfloat16' in torch_dtype:
        model = model.bfloat16()
    elif 'float16' in torch_dtype:
        model = model.half()
    model.save_pretrained(args.output_dir, safe_serialization=False)
    print(f'Save weights finished. take: {time.time() - start_time:.3f}s\n')
