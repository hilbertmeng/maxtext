...
def forward_kld_topk(logits, teacher_logits, k: int):
    ...
    top_k_mask = jnp.sum(jax.nn.one_hot(top_k_indices, num_classes, dtype=jnp.float32), axis=-2)
    ...
    prod_probs = safe_student_logprobs * effective_teacher_probs
...
def reverse_kld_topk(logits, teacher_logits, k: int):
    ...
    prod_probs = safe_student_probs * effective_teacher_logprobs
    distill_xent = -jnp.sum(prod_probs, axis=-1)
    return distill_xent
