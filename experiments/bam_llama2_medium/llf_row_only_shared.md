...
实验类：`BamMediumIndependentLLFBAlignedRowLocalVRowSharedColRank4B`。计划 RUN 同名，正式训练 TPU `xd-v5p-16-llf-row-shared-maxtext`，主区 `europe-west4-b`，v5p-16，13500 步，checkpoint 每 200 步，Generic health ON、BAM sow OFF。
...
验证：新增 LocalV 模式测试 13 项通过，包括本配置在 L 层的前传、参数和非零梯度，以及 F 层不含新增参数；固定 CPU BAM 测试套件待完成。正式启动以 target-topology AOT、`Loaded compiled function!` 和 `FIRST_STEP` 为门槛。
