...
def run(root,stage='targeted'):
 ...
 control_index=next(i for i,s in enumerate(scenarios) if s['name'] in ('QK_off','QK_all_off'))
 for i in range(64):
  ...
  with np.load(file) as a,np.load(next(root.glob(f'worker*/all_{i:03d}.npz'))) as b:
   ...
   np.testing.assert_array_equal(a['gap'][control_index],b['gap'][3])
...
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('root',type=Path);p.add_argument('--stage',choices=('targeted','depth'),default='targeted');a=p.parse_args();print(json.dumps(run(a.root,a.stage),indent=2))
