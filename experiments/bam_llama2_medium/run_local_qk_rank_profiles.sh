...
for exp_class in "$@"; do
  ...
  "$COLLECTOR" "$TPU" "$ZONE" "$remote_run/tensorboard" \
    "$gcs_run" "$PROJECT" 2 0 >"$collector_log" 2>&1 &
  collector_pid=$!

  gcloud compute tpus tpu-vm ssh --internal-ip "$TPU" --zone="$ZONE" \
    --project="$PROJECT" --worker=all \
    --command="cd '$REMOTE_REPO'; nohup env SMOKE_OUTPUT='$remote_run' \
    '$SMOKE' '$exp_class' '$run' 100 >'$train_log' 2>&1 </dev/null &"
  ...
  while (( SECONDS < deadline )); do
    status=$(gcloud compute tpus tpu-vm ssh --internal-ip "$TPU" --zone="$ZONE" \
      ...
      then echo RUNNING; else echo EXITED; fi" 2>/dev/null | tail -1)
    case "$status" in
      ...
      ERROR|EXITED)
        ...
        gcloud compute tpus tpu-vm ssh --internal-ip "$TPU" --zone="$ZONE" \
          --project="$PROJECT" --worker=0 \
          --command="tail -100 '$train_log'" >&2 || true
        break ;;
    esac
    sleep 5
  done
  ...
  gcloud compute tpus tpu-vm ssh --internal-ip "$TPU" --zone="$ZONE" \
    --project="$PROJECT" --worker=0 \
    --command="grep 'completed step:' '$train_log' | tail -6" || true
