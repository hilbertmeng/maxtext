...
def match_unused_content(line):
    if poison_content.search(line.strip()) or ahthor_pat.search(line.strip()) or match_chapter(line.strip()):
    # if poison_content.search(line) or ahthor_pat.search(line) or chapter_digit.search(line):
        return True
    else:
        return False
...
class QwenTokenizer():
    def __init__(self, tokenizer_path, save_dir, rank):
        ...
        self.extract_max_nums_perline = 2
...
def process_data(args):
    ...
    for path in cur_rank_pathes:
        ...
        for i in tqdm(range(len(cur_rank_lines)), desc=f'Rank-{rank}'):
            qwen_tokenizer.extract_nums_perline = 0 # 每本书初始计数0
            ...
            if len(text_split) > per:
                ...
                for lnx in tqdm(range(0, len(text_split), per), desc=f'Rank-{rank}-sub-{i}'):
                    ...
                    # 有些很长的书，部分书对其截断? or random.randint(0, 1)
...
if __name__ == "__main__":
    ...
    if data_type == 'valid':
        ...
        SAVE_DIR = f'gs://newproject-1-jax_llm_data_us-east5/xiaomeng/v3.5mini/unigram_32k_tfids0601/validation'
        print(f'SAVE_DIR: {SAVE_DIR}')
    else:
        buckets = args.bucketes.split('-')
        bucket_start = int(buckets[0])
        bucket_end = int(buckets[1]) if len(buckets) == 2 else bucket_start  + 1
        SAVE_DIR = f'gs://newproject-1-jax_llm_data_us-east5/xiaomeng/v3.5mini/unigram_32k_tfids0601/B0-40'
...
多进程处理多个文件:
# Usage:
TPU_NAME=llm-jax-v6e-1-10; ZONE=europe-west4-a
...
# scp
TPU_NAME=llm-jax-v6e-1-10; ZONE=europe-west4-a
SCRIPT=/Users/lishengping/codes/jax_projects/maxtext/scripts/extract_book_start.py
...
gcloud compute tpus tpu-vm ssh $TPU_NAME --zone=$ZONE --worker=0 --command="killall processed.py;/home/lishengping/miniconda3/bin/python processed.py --bucket 0-40 --workers 40 --rank_start_index 0 --data_type train" --project=newproject-1-451205
