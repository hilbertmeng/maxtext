...
import numpy as np
from layers.attentions import _BamReadArm, factorized_head_bam_read


def _effective_key_bam_read(M, key, mix, gate_logits, *, v_projection=None,
                            return_sides=True, **settings):
  """Exercise scheme C through the unified production read entry point."""
  arm = _BamReadArm(
      name='t', k_dim=M.shape[-2], v_dim=M.shape[-1], num_heads=mix.shape[-3],
      rank=key.shape[-2], rank_routing='effective_key', key_mode='rms_gate', **settings)
  y = factorized_head_bam_read(M, key, mix, arm, gate_logits=gate_logits,
      ...
      def run(args, gram):
        return _effective_key_bam_read(
            *args, rms_epsilon=1e-4, key_scale=2., implementation='mul_reduce_btn',
            ...
            read_side='both', v_projection=None,
            return_sides=False)
      ref = run(tuple(x.astype(jnp.float32) for x in args), 'dot')
      for gram in ('dot', 'mul_reduce'):
        out = run(args, gram).astype(jnp.float32)
        rel = float(jnp.linalg.norm(out - ref) / jnp.linalg.norm(ref))
        print(f'bf16 rank={rank} gram={gram} relative_L2={rel:.6g}', flush=True)
