...
def generate(self, input_ids, decode_one_token, **kwargs):
    ...
    if not prefill_compile:
        ...
        input_ids = torch.nn.functional.pad(input_ids, pad=[0, pad])
        input_pos = torch.arange(seq_length + pad, device=self.device)
        logits = self.forward(input_ids, input_pos=input_pos, return_tensor=True)
        last_logit = logits[:, -pad - 1]
    else:
    # ============================segment compiled forward start============================
        ...
        # input_pos = torch.nn.functional.pad(input_pos, pad=[prefill_pad_length])
        ...
        for c in range(chunks):
            chunk_input_ids = input_ids[:, c * prefill_chunk_size: (c + 1) * prefill_chunk_size]
            print(f'chunk_input_ids: {chunk_input_ids.shape}')
            chunk_input_pos = input_pos[c * prefill_chunk_size: (c + 1) * prefill_chunk_size]
            print(f'chunk_input_pos: {chunk_input_pos}')
            logits = compiled_prefill_func(self, chunk_input_ids, input_pos=chunk_input_pos, return_tensor=True)
        print(f'logits: {logits.shape}')
        last_logit = logits[:, -prefill_pad_length - 1]
        # ============================segment compiled forward end============================
