...
case "$model" in
  ...
  xl)
    ...
    trainer=aef0d97411a1725386ebba1aeae1bf4acb1bb79e
    batch=1 ;;
  *) exit 2 ;;
esac
tag="bam-first6-self-only-$model-${commit:0:7}"
output="/tmp/$tag"
...
env HARDWARE=tpu JAX_TRACEBACK_FILTERING=off \
    DIAGNOSTIC_COMMIT="$commit" BAM_SELF_ONLY_OUTPUT="$output" \
    ...
    "$python" experiments/bam_llama2_medium/first6_self_only.py \
    MaxText/configs/base.yml exp_class=BamFirst6SelfOnly run_name="$tag" \
    load_parameters_path="$checkpoint" base_output_directory="$output/maxtext-output" \
    tensorboard_dir="$output/tensorboard" only_eval=True \
    dataset_path=gs://newproject-1-common_datasets_europe-west4/pythia_pile_idxmaps_tfrecord \
...
wait "$pid"
upload
echo "SELF_ONLY_UPLOADED model=$model gcs=$gcs"
