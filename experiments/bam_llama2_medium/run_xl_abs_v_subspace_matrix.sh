...
CODE_COMMIT="${BAM_ABSV_SUBSPACE_DIAGNOSTIC_COMMIT:-working}"
COHORT_PATH="${BAM_ABSV_SUBSPACE_COHORT_PATH:-/tmp/pile-eval-t2048-seed9876-n128-v1.npz}"

gsutil cp "$COHORT_GCS" "$COHORT_PATH"

run_one() {
  local config="$1" trainer_commit="$2" step="$3" short_name="$4" ranks="$5"
  local checkpoint="gs://newproject-1-llm_projects_europe-west4/log/$config/checkpoints/$step/items"
  local run="xl-rank2-${short_name}-s${step}-subspace-pile128-${CODE_COMMIT:0:7}"
  local output="/tmp/$run"
  mkdir -p "$output/output/$run" "$output/tensorboard"
  env \
    HARDWARE=tpu \
    JAX_TRACEBACK_FILTERING=off \
    BAM_ABSV_SUBSPACE_RANKS="$ranks" \
    BAM_ABSV_SUBSPACE_BATCH_SIZE=16 \
    BAM_ABSV_SUBSPACE_TRAINER_COMMIT="$trainer_commit" \
    BAM_ABSV_SUBSPACE_DIAGNOSTIC_COMMIT="$CODE_COMMIT" \
    BAM_ABSV_SUBSPACE_COHORT_PATH="$COHORT_PATH" \
    BAM_ABSV_SUBSPACE_OUTPUT="$output/report.json" \
    "$PYTHON" experiments/bam_llama2_medium/xl_abs_v_subspace_ablation.py \
