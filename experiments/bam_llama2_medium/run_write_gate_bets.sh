...
export BET_OUTPUT=/tmp/alllocal-write-gate-bets-0922
export BET_PROTOCOL=$BET_OUTPUT/protocol.json
GCS=gs://newproject-1-llm_projects_europe-west4/log/diagnostics/alllocal-write-gate-bets-0922
...
while kill -0 "$job" 2>/dev/null; do
 sleep 30
 gsutil -m rsync -r -x '(^|/)(tb/.*|maxtext-output/.*)' "$BET_OUTPUT" "$GCS/worker" || true
done
result=0
wait "$job" || result=$?
gsutil -m rsync -r -x '(^|/)(tb/.*|maxtext-output/.*)' "$BET_OUTPUT" "$GCS/worker"
exit "$result"
