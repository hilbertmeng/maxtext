...
from layers.attentions import (factorized_head_bam_read, _basis_gram,
    _contract_bam_read_sides, _BamReadArm, _packed_local_layout, _packed_local_arms_init, BamAttention)
...
class SharedBasisTest(unittest.TestCase):
  ...
  def test_xl_module_and_scan_shape(self):
    ...
    for params in (p0, p1):
      self.assertIn('bias', params['P_loc_up'])
      for retired in ('P_agg_u', 'S_v', 'write_v_mix_scale', 'write_v_bias',
                      'abs_k_cache_projection', 'abs_k_col_decoder'):
    ...
    arms=[_BamReadArm(name=n,rank=4,k_dim=64,v_dim=32,rank_routing='effective_key',num_heads=2) for n in ('q','k','v')]
  ...
  def test_cached_pair_forward_and_gradient(self):
    ...
    for impl in ('dot', 'mul_reduce'):
      arm=_BamReadArm(name='t',k_dim=6,v_dim=5,num_heads=2,rank=4,rank_routing='effective_key',
                      key_scale=2.,rms_epsilon=1e-4,gram_implementation=impl)
      def pair(m,a,h,g,cached):
        row,col=jnp.split(a,[6],-1)
        cache=(_contract_bam_read_sides(m,m,row,col,'mul_reduce_btn','both'),
               tuple(_basis_gram(z,arm) for z in (row,col))) if cached else None
        return tuple(factorized_head_bam_read(m,a,h[i],arm,gate_logits=g[i],
            basis_cache=cache) for i in range(2))
  ...
  def test_packed_slices_keep_independent_mix_initialization(self):
    arms=[_BamReadArm(name=n,rank=4,k_dim=6,v_dim=5,rank_routing='effective_key',num_heads=2) for n in ('q','k','v')]
