#!/usr/bin/env python3
"""Bounded standalone JIT timing; run on tpu-ag, never starts auto-train."""
import argparse
import json
...
def main():
    ...
    for suffix in ('LocalVLLF', 'SharedReadLLF'):
        exp = prefix + suffix
        run = 'TimingJitLLF_' + suffix + '_' + str(int(time.time()))
        log = '/home/lishengping/train_' + run + '.log'
        pattern = '[M]axText/train.py.*run_name=' + run
        arm = {'exp': exp, 'run': run, 'log': log, 'state': 'launching'}
        result['arms'].append(arm)
        save()
        # The existing smoke entry preserves the class's 50k schedule; no checkpoint
        # or profiler is needed for comparison with historical log steps 10..14.
        smoke = repo + '/.claude/skills/tpu-diagnostics/scripts/run_train_smoke.sh'
        command = ('cd ' + shlex.quote(repo) + '; nohup env DATASET_PATH=' + shlex.quote(dataset)
                   ...
                   + ' 50000 >' + shlex.quote(log) + ' 2>&1 </dev/null &')
        ssh(command, 'all')
