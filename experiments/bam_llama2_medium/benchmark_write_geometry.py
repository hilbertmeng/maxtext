...
root=Path(sys.argv[1]);n=json.loads((root/'metadata.json').read_text())['n'];tasks=[(str(root),l,n,.1,False) for l in [3,12]]
t=time.perf_counter();serial=[analyze_layer(t) for t in tasks];serial_seconds=time.perf_counter()-t
arrays=[]
for l in [3,12]:
 with np.load(root/f'analysis_gate010/layer_{l:02d}.npz') as f:arrays.append({k:f[k] for k in f.files})
t=time.perf_counter()
...
for l,expected in zip([3,12],arrays):
 with np.load(root/f'analysis_gate010/layer_{l:02d}.npz') as f:
