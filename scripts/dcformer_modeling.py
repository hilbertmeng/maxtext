...
class KVKWCache(nn.Module):
    ...
    def update(self, input_pos, k_val, v_val, kw_val=None, kw_sub=None): # kw_val B,N,S,2,N      B2NSD
        ...
        if self.window_size is None:
            ...
            if self.use_kw_cache and kw_sub is not None:
                kw_sub_out[:, input_pos] = kw_sub
            ...
            #  self.k_cache = torch.cat([k_out[:, :, start: ].clone(), k_val], dim=2)
            #  self.v_cache = torch.cat([v_out[:, :, start: ].clone(), v_val], dim=2)

            #  if self.use_kw_cache and kw_val is not None:
            #      self.kw_cache = torch.cat([kw_out[:, start: ].clone(), kw_val], dim=1)

            #  if self.use_kw_cache and kw_sub is not None:
            #      self.kw_sub_cache = torch.cat([kw_sub_out[:, start: ].clone(), kw_sub], dim=1)

           # start = max(0, input_pos[-1]-self.seq_length+1)
             value_pos = input_pos % self.window_size
             input_pos = input_pos % self.seq_length
             v_out[:, :, input_pos] = v_val[:,:,value_pos]
             k_out[:, :, input_pos] = k_val[:,:,value_pos]
             if self.use_kw_cache and kw_val is not None:
                 kw_out[:, input_pos] = kw_val[:,value_pos]

             if self.use_kw_cache and kw_sub is not None:
                 kw_sub_out[:, input_pos] = kw_sub[:, value_pos]
        return k_out, v_out, kw_out, kw_sub_out
       # return self.k_cache, self.v_cache, self.kw_cache, self.kw_sub_cache
...
class DCFormer(PreTrainedModel):
    ...
    def setup_caches(self, max_batch_size, max_seq_length, set_kv_cache=True):
        ...
        if self.is_training:
            ...
            global_mask = torch.tril(torch.ones(self.max_seq_length, self.max_seq_length, dtype=torch.bool))
...
class DCMHAttention(nn.Module):
    ...
    def _generate_fast(self, x, input_pos, q, k, v, k_mask):
        ...
        if self.kv_cache is not None:
            k, v, kw_out, _ = self.kv_cache.update(input_pos, k, v, kw_val=kw_new) #BNT2M
        ...
        if self.is_training:
            ...
            if seqlen == 1: # one-token generation
                ...
                if fast_infer:
                    ...
                    if self.kv_cache is not None:
                        k, v, kw_out, _ = self.kv_cache.update(input_pos, k, v, kw_val=kw_new) # BNSD, BNSD, BS2NN
 ...
 #                   probs = probs.to(torch.float16) # lsp: add
  ...
  #                  probs *= k_mask # lsp: add
                ...
                if self.kv_cache is not None:
                    k, v, _, kw_sub_out = self.kv_cache.update(input_pos, k, v, kw_val=kw_new, kw_sub=kw_sub) # BNSD, BNSD, BS2NN
                ...
                logits = torch.where(k_mask, logits, torch.finfo(torch.float16).min) # lsp: float16 -> float32
                probs = logits.softmax(-1)
    #            probs = probs.to(torch.float16) # lsp: add
