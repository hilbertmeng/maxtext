...
### Mechanism comparison versus original sigmoid dual-write (user priority)

At600 L2 read-gate head median .1799 -> .0017 and actualfeedbacknormshare .5409 -> .9660; L3 read .1364 -> .0002, feedbackshare .9468 -> .8452. This is suppression of direct LocalO residual injection while retaining memory feedback, NOT no use of M (shared LocalV still exists). Middle read median .06267 -> .02941; feedbacknormshare median .33739 -> .41816, but per-layer mean component energyshare .39708 -> .38999. Do not conflate these aggregation metrics. Middle normshare head10–90% .0236–.9416 -> .0793–.8186; late .2169–.8024 -> .2499–.6266: fewer extremes, consistent with separate RMS removing magnitude variation, not causal proof of loss trend. Read/main corr median middle -.1757 -> -.0325; independent kernel initialization also changes initial correlations, so not isolated normalization evidence. Gradients600 W_R ratio1.19x, main gate1.56x, feedbackgate1.30x; no evidence the new gate is starved.

Artifacts independent_edges_mechanism.json, independent_edges_mechanism_600.png/pdf and *_trajectory.png/pdf under /data0/xd/bam_diagnostics/dual_write_training; plotting script checked visually. User explicitly wants health comparisons against original dual-write. Independent800 gap+.003591 (crossing from600-.001489); continue200-step anomaly review. AddressMix EW4a compiler preempted once; controller requeued-r1, UC1a/UE5a queues retained.

Independent1000 gap+.005228 vsoriginaldual (800+.003591): early gain has reversed; not stable benefit. Middle read/main/feedback/normshare original=.055998/.114441/.090293/.251824 vsindependent=.030922/.106480/.055104/.345472. Mean headshares .38731 vs.39552, so medians do not imply equivalently large aggregate feedback increase. L2 read .16512->.00369, share.59617->.96753; L3 read.10754->.00039, share.94958->.86645. Globalgrad .40513 vs.52873.

User asked whether AddressMix fully decouples everything: no. lambda routes a common feedback budget (positive coefficient wR*lambda, erase wR*(1-lambda)); interpolation norm depends on endpoint angle; read content/address share C and q, main/feedback normal-write endpoint shares p. Proposed future replacement with independently gated positive and negative terms uses same two gate projections but no shared budget. Not authorized/implemented; continue current AddressMix as requested.

### Independent erasure follow-up authorized

User explicitly authorized the recommended independent positive-feedback and negative-read erasure variant, alongside AddressMix. RUN `BamMediumAllLocalIndependentErase`, same worktree/branch, from scratch13500/review2800. Formula `(wu*Norm(u)+wf*Norm(R)) outer Norm(p) - we*Norm(R) outer Norm(C q)`. No shared positive/negative budget; two outer products and one M update. New erase sigmoid uses original main initializer with independent RNG and same logit(.1) bias. Same parameter/M-cache cost as AddressMix (+.0156403W_Q per layer vsIndependentEdges). Main direct baselines AddressMix and originalDual, health always same-step originalDual comparisons. Bet: selective erase in some layer/heads rather than uniformly large erase, potential small2800 gain (-.003 vsAddressMix), early trajectory uncertain; speed same order asAddressMix (within1% with matchedhealth).

Health: erase mean/std/nearzero/saturation, positive and erase component normshares, erase energyshare, net LocalO share and actual positive/negative/net record RMS, read/new cosine, erase/feedback correlation. Existing dual metrics describe positive feedback relative to main+positive only; new3-component metrics and netLocalO metrics must be used for total allocation comparisons. Total9120 BAM tags vsAddressMix7080 vsIndependent3816; genericON, unmatched formal speed timing flagged.

AOT resource recovery: AddressMix EW4a attempts0/1 preempted just after install; r2 active acquisition. PassiveFlex `xd-v6e-aot-c5be705-flex-ue5a`, UE5a, 1h, accepted07:23UTC using gcloud enumFLEX_START (lowercaseflex-start rejected locally before submission). Started standard standalone installer to adopt accepted queue; owned by this task, MUST delete if spot wins, or hand compilation over when INSTALL_OK. Existing spot UC1a/UE5a queues preserved. No diagnostic machines touched.

IndependentErase focused test passed75.931s; full53-test suite running. Independent1200 gap+.007234 vsdual, median middle feedbackgate .05102 vs.08967 but normshare.31660 vs.23014, continuing slow shift toward main writes. Originaldual11000 recent5gapvsAllLocal-.001176 (range-.002042..-.000506), normsharemiddle.10259 stable, main.18871.

IndependentErase full pinned CPU suite passed: Ran 53 tests in 376.255s; /tmp/independent_erase_full_tests.log. No runtime changes after validation. Independent1600 gap+.003503 (1200+.007234,1400+.005111), mild recovery not established win. Middle feedbacknormshare original .19414 vsindependent .28677; main gates.13332 vs.12218, feedback.08638 vs.04849, read.05587 vs.03212.
