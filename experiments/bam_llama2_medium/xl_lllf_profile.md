...
Canonical report and analysis scripts: main `refactor-bam`,
`/home/xd/projects/maxtext/experiments/bam_llama2_medium/`.
Model implementation remains in the experimental worktree; importing this report
and its standalone parsers does not merge experimental model changes.
...
Parser: main `experiments/bam_llama2_medium/analyze_bam_xplane.py` (also retained in
...
Promising follow-up: gate the compact row/column outputs before head padding,
retain the two consumers while minimizing backward split/concatenate/copies, and
